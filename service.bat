...
if "%~1"=="check_updates" (
    if exist "%~dp0utils\check_updates.enabled" (
        if /i "%~2"=="soft" (
            ...
            set "DGEN_UPD_LOCKDIR=%TEMP%\dgen-check-updates.lock"
            mkdir "!DGEN_UPD_LOCKDIR!" 2>nul
            if errorlevel 1 (
                endlocal
                exit /b
            )

            set "DGEN_UPD_STAMP=%TEMP%\dgen-check-updates.stamp"
            for /f %%A in ('powershell -NoProfile -Command "Get-Date -Format yyyyMMdd"') do set "DGEN_UPD_TODAY=%%A"

            if exist "!DGEN_UPD_STAMP!" (
                set "DGEN_UPD_LAST="
                set /p DGEN_UPD_LAST=<"!DGEN_UPD_STAMP!" 2>nul
                if "!DGEN_UPD_LAST!"=="!DGEN_UPD_TODAY!" (
                    rmdir /s /q "!DGEN_UPD_LOCKDIR!" >nul 2>&1
                    endlocal
                    exit /b
                )
            )

            >"!DGEN_UPD_STAMP!" echo !DGEN_UPD_TODAY!
...
sc query "DGen" >nul 2>&1
if !errorlevel!==0 (
    for /f "tokens=2*" %%A in ('reg query "HKLM\System\CurrentControlSet\Services\DGen" /v DGen-discord-youtube 2^>nul') do echo Service strategy installed from "%%B"
)

call :test_service DGen
...
tasklist /FI "IMAGENAME eq DGen.exe" | find /I "DGen.exe" > nul
if !errorlevel!==0 (
    call :PrintGreen "Bypass (DGen.exe) is RUNNING."
) else (
    call :PrintRed "Bypass (DGen.exe) is NOT running."
)
...
set "count=0"
set "STRATEGIES_DIR=%~dp0strategies"
for %%f in ("%STRATEGIES_DIR%\general*.bat") do (
    set /a count+=1
    echo !count!. %%~nxf
    set "file!count!=%%~f"
)
...
for /f "tokens=*" %%a in ('type "!selectedFile!"') do (
    ...
    echo !line! | findstr /i "%BIN%DGen.exe" >nul
    if not errorlevel 1 (
        set "capture=1"
    )
...
sc create %SRVCNAME% binPath= "\"%BIN_PATH%DGen.exe\" !ARGS!" DisplayName= "D-Gen" start= auto
sc description %SRVCNAME% "D-Gen DPI bypass software"
sc start %SRVCNAME%
for %%F in ("!file%choice%!") do (
    set "filename=%%~nF"
)
reg add "HKLM\System\CurrentControlSet\Services\DGen" /v DGen-discord-youtube /t REG_SZ /d "!filename!" /f
...
if /i "%1"=="soft" goto service_check_updates_soft
chcp 437 > nul
cls

set "GITHUB_VERSION_URL=https://raw.githubusercontent.com/ViZuaaaaa/NoDPI-D-Gen/main/.service/version.txt"
set "GITHUB_RELEASE_URL=https://github.com/ViZuaaaaa/NoDPI-D-Gen/releases/latest"
...
if not defined GITHUB_VERSION (
    echo Warning: failed to fetch the latest version. This warning does not affect the operation of DGen
    timeout /T 9
    if "%1"=="soft" exit 
    goto menu
)
...
set "CHOICE="
set /p "CHOICE=Open the release page? (Y/N) (default: Y) "
if "%CHOICE%"=="" set "CHOICE=Y"
if /i "%CHOICE%"=="y" set "CHOICE=Y"

if /i "%CHOICE%"=="Y" (
    echo Opening the download page...
    start "" "%GITHUB_RELEASE_URL%"
)
...
if not defined GITHUB_VERSION (
    echo Warning: failed to fetch the latest version (soft mode). Skipping update check.
    exit /b
)

if "%LOCAL_VERSION%"=="%GITHUB_VERSION%" exit /b
...
if !errorlevel!==0 (
    ...
    call :PrintRed "[X] Base Filtering Engine is not running. This service is required for DGen to work"
)
echo:
...
if !errorlevel!==0 (
    ...
    call :PrintRed "https://github.com/ViZuaaaaa/NoDPI-D-Gen/issues"
) else (
...
if !errorlevel!==0 (
    call :PrintRed "[X] Killer services found. Killer conflicts with DGen"
    call :PrintRed "https://github.com/ViZuaaaaa/NoDPI-D-Gen/issues"
) else (
...
if !errorlevel!==0 (
    call :PrintRed "[X] Intel Connectivity Network Service found. It conflicts with DGen"
...
if !checkpointFound!==1 (
    call :PrintRed "[X] Check Point services found. Check Point conflicts with DGen"
...
if !errorlevel!==0 (
    call :PrintRed "[X] SmartByte services found. SmartByte conflicts with DGen"
...
if !errorlevel!==0 (
    ...
    call :PrintYellow "[?] VPN services found:!VPN_SERVICES!. Some VPNs can conflict with DGen"
...
tasklist /FI "IMAGENAME eq DGen.exe" | find /I "DGen.exe" > nul
set "dgen_running=!errorlevel!"

sc query "WinDivert" | findstr /I "RUNNING STOP_PENDING" > nul
set "windivert_running=!errorlevel!"

if !dgen_running! neq 0 if !windivert_running!==0 (
    call :PrintYellow "[?] DGen.exe is not running but WinDivert service is active. Attempting to delete WinDivert..."
...
set "conflicting_services=GoodbyeDPI discordfix_DGen winws1 winws2"
...
if not exist "%gameFlagFile%" (
    ...
    call :PrintYellow "Restart the DGen to apply the changes"
) else (
...
set "listFile=%~dp0lists\ipset-all.txt"
set "url=https://raw.githubusercontent.com/ViZuaaaaa/NoDPI-D-Gen/refs/heads/main/.service/ipset-service.txt"
...
if exist "%SystemRoot%\System32\curl.exe" (
    ...
    powershell -NoProfile -Command ^
        ...
        "try { Invoke-WebRequest -Uri $url -TimeoutSec 10 -UseBasicParsing -OutFile $out; exit 0 } catch { exit 1 }"
)
...
set "hostsUrl=https://raw.githubusercontent.com/ViZuaaaaa/NoDPI-D-Gen/refs/heads/main/.service/hosts"
set "tempFile=%TEMP%\DGen_hosts.txt"
set "needsUpdate=0"
...
echo.
start "" powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0utils\test DGen.ps1"
pause
goto menu
...
if %errorLevel% neq 0 (
    ...
    echo Fix your PATH variable with instructions here https://github.com/ViZuaaaaa/NoDPI-D-Gen/issues
    pause
    exit /b 1
)
exit /b 0
...
if "%extracted%"=="0" (
    echo DGen must be extracted from archive first or bin folder not found for some reason
    pause
    exit
)
exit /b 0