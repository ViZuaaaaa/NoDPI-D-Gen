...
function Test-DGenServiceConflict {
    return [bool](Get-Service -Name "DGen" -ErrorAction SilentlyContinue)
}
...
# Check if DGen service installed
if (Test-DGenServiceConflict) {
    Write-Host "[ERROR] Windows service 'DGen' is installed" -ForegroundColor Red
...
$batFiles = Get-ChildItem -Path $strategiesDir -Filter "general*.bat" -File | Sort-Object Name
...
if (-not $batFiles -or $batFiles.Count -eq 0) {
    Write-Host "[ERROR] No strategy .bat files found in strategies/ folder" -ForegroundColor Red
    Write-Host "Press any key to exit..." -ForegroundColor Yellow
    [void][System.Console]::ReadKey($true)
    exit 1
}
...
# Stop DGen
function Stop-DGen {
    Get-Process -Name "DGen" -ErrorAction SilentlyContinue | Stop-Process -Force
}

# Capture/restore running DGen instances to return user ipset/config
function Get-DGenSnapshot {
    try {
        return Get-CimInstance Win32_Process -Filter "Name='DGen.exe'" |
...
function Restore-DGenSnapshot {
    param($snapshot)

    if (-not $snapshot -or $snapshot.Count -eq 0) { return }

    $current = @()
    try { $current = (Get-DGenSnapshot).CommandLine } catch { $current = @() }

    Write-Host "[INFO] Restoring previously running DGen instances..." -ForegroundColor DarkGray
...
$originalDGen = Get-DGenSnapshot

Write-Host ""
Write-Host "============================================================" -ForegroundColor Cyan
Write-Host "                 DGen CONFIG TESTS" -ForegroundColor Cyan
...
try {
    ...
    Stop-DGen
    Restore-DGenSnapshot -snapshot $originalDGen
    if ($originalIpsetStatus -ne "any") {
